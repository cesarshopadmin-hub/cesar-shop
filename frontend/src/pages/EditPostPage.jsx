  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setPhoneInputVal((data.countryCode || "20") + (data.whatsappNumber || ""));
        if (data.images) {
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <div className={`flex rounded-xl border overflow-hidden transition ${
                ...
                {/* Static EGP badge */}
                <div className="shrink-0 flex items-center justify-center bg-white/5 border-r border-white/10 px-4 py-3 text-cesar-cyan font-black text-sm select-none">
                  ج.م
                </div>
                {/* Price input */}
                <input
                  ...
                  onChange={handleChange}
                  placeholder="0"
                  className="w-full bg-black/40 px-4 py-3 text-white outline-none"
                />
              </div>
              {fieldErrors.price && (
